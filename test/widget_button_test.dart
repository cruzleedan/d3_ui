  ...
  group('D3Button', () {
    testWidgets('renders label text', (tester) async {
      await tester.pumpWidget(
        _wrap(D3Button(label: 'Hello', onPressed: () {})),
      );
    ...
    testWidgets('calls onPressed when tapped', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(D3Button(label: 'Tap me', onPressed: () => tapped = true)),
      );
    ...
    testWidgets('does not call onPressed when null (disabled)', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(const D3Button(label: 'Disabled', onPressed: null)),
      );
    ...
    testWidgets('icon-only variant renders without label', (tester) async {
      await tester.pumpWidget(
        _wrap(D3Button.icon(icon: Icons.add, onPressed: () {})),
      );
    ...
    testWidgets('isFullWidth stretches to available width', (tester) async {
      await tester.pumpWidget(
        _wrap(D3Button(label: 'Full', isFullWidth: true, onPressed: () {})),
      );
      final sizedBox = tester.widget<SizedBox>(
        find
            .ancestor(of: find.text('Full'), matching: find.byType(SizedBox))
            .first,
      );
    ...
    testWidgets('leading icon is rendered', (tester) async {
      await tester.pumpWidget(
        _wrap(D3Button(label: 'Add', leadingIcon: Icons.add, onPressed: () {})),
      );
      expect(find.byIcon(Icons.add), findsOneWidget);
    });
    ...
    testWidgets('all sizes render without error', (tester) async {
      for (final size in D3ButtonSize.values) {
        await tester.pumpWidget(
          _wrap(D3Button(label: size.name, size: size, onPressed: () {})),
        );
